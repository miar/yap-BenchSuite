...
linear(Depth):-
   atomic_concat(['data_edge_linear_',Depth,'.pl'],File),
   atomic_concat(['Linear configuration with depth ',Depth],Program),
   open(File,'write',FO),
   tell(FO),
   write_header(File,Program),
   edge_linear(Depth,_),
...
edge_linear(1,b):- 
   write_edge(0,a,1), !.
edge_linear(K,NewSymbol):-
   K1 is K-1,
   edge_linear(K1,Symbol),
   write_edge(K1,Symbol,K),
   (Symbol = a -> NewSymbol = b ; NewSymbol = a).
...
write_edge(X1,X2) :- 
   write('edge('), write(X1), write(','), write(X2), write(').'), nl.
